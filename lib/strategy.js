    ...
    try {
      challengeFunction(address, (err, res = '') => {
        try {
          ...
          const resultAddress = web3.eth.accounts.recover(`${_instance._key}${res}`, encryptedChallenge);
          ...
          checkNftBalance().then(balance => {
            ...
            function verified(_err, user, info) {
              if (_err) return _instance.fail(_err);
            ...
            try {
              ...
              return _instance.fail(__err);
            }
          }).catch(_err => {
            _instance.fail({ message: _err.message });
          });
        } catch (_err) {
          _instance.fail({ message: _err.message });
        }
