...
class NFTStrategy extends PassportStrategy {
  ...
  authenticate(req) {
    ...
    const address = req.get(_instance._addressField);
    const encryptedChallenge = req.get(_instance._challengeField);
    ...
    try {
      challengeFunction(address, (err, res = '') => {
        ...
        web3.eth.personal.ecRecover(`${_instance.key}${res}`, encryptedChallenge, (_err, resultAddress) => {
          if (_err) throw _err;
          if (resultAddress !== address) throw new Error(`[NFTStrategy] Address mismatch?! ${address}`);
          ...
          checkNftBalance().then(balance => {
            ...
            try {
              if (_instance._passReqToCallback) {
                this._verify(req, balance, verified);
              } else {
                this._verify(balance, verified);
              }
            } catch (err) {
